	...
	public void execute() {
		...
		while (code != STOP ) {
			...
			case ILOAD:
				...
				data.push(method.getVarValues().get(n));break;
			case ICONST: // push integer value arg
				data.push(Integer.parseInt(arg.toString()));break;
			case ISTORE:
				n = Integer.parseInt(arg.toString());
				method.getVarValues().set(n, data.pop());
				break;
			case IADD: // v1 = pop(), v2 = pop(), push(v1+v2)
				data.push( data.pop()+ data.pop() ); break;
			case IMUL:
				data.push( data.pop()* data.pop() ); break;
			case ISUB:
				Integer n1 = data.pop();
				Integer n2 = data.pop();
				data.push(n2 - n1);break;
			...
			case GOTO: // pc = arg 
				n = Integer.parseInt(arg.toString());
				next = n-1; break;//at the end of switch adding is happened then i decrease to pc-1
			...
			case IRETURN:
				...
				break;
			case PRINT:
				System.out.println(data.pop());break;
			}
			next++;
		}
	}

	public ClassFile readFromFile(String name) throws IOException, ClassNotFoundException {
		ObjectInputStream objectIn = null;
		objectIn = new ObjectInputStream(new BufferedInputStream(new FileInputStream(name)));
		ClassFile clsfile = (ClassFile) objectIn.readObject();
		objectIn.close();
		return clsfile;
	}
}
